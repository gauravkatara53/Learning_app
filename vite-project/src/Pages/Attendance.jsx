  ...
  const calculateAttendance = (e) => {
    ...
    if (isNaN(attendancePercentage)) {
      ...
      if (attendancePercentage < desired * 100) {
        const requiredClasses = Math.ceil((desired * total - attended) / (1 - desired));
        resultMessage = `
          <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
              <p>Your attendance is ${attendancePercentage.toFixed(2)}%</p>
              <p>You need to attend ${requiredClasses} more classes to reach ${desired * 100}% attendance.</p>
          </div>
        `;
      } else {
        ...
        resultMessage = `
          <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative" role="alert">
              <p>Your attendance is ${attendancePercentage.toFixed(2)}%</p>
              <p>You can miss ${maxMissableClasses} more classes and still maintain ${desired * 100}% attendance.</p>
          </div>
        `;
      }
    ...
    <div className="bg-black text-white flex items-center justify-center min-h-screen">
      <div className="w-full max-w-md mx-auto p-4">
        <div className="bg-black p-8 rounded shadow-md w-full">
          <h1 className="text-4xl font-semibold text-center mb-2">Attendance Calculator</h1>
          <p className="text-center text-gray-400 mb-8">You can enter any desired percentage you want</p>
          <form id="attendanceForm" className="space-y-6" onSubmit={calculateAttendance}>
            <div>
              <label htmlFor="totalClasses" className="block text-lg font-bold mb-2">Total Classes:</label>
              <input
                type="number"
                id="totalClasses"
            ...
            <div>
              <label htmlFor="attendedClasses" className="block text-lg font-bold mb-2">Attended Classes:</label>
              <input
                type="number"
                id="attendedClasses"
                className="shadow appearance-none border border-gray-700 rounded w-full py-3 px-4 leading-tight focus:outline-none focus:shadow-outline bg-black text-white text-lg"
            ...
            <div>
              <label htmlFor="desiredAttendance" className="block text-lg font-bold mb-2">Desired Attendance (%):</label>
              <input
                type="number"
          ...
          <div id="result" className="mt-6 text-lg" dangerouslySetInnerHTML={{ __html: result }} />
        </div>
        <div className="flex justify-center mt-8">
          <p className="text-gray-400">Made with ❤️ and code by &nbsp;</p>
          <p><a className="font-bold text-blue-400" href="https://github.com/gauravkatara53/attendance_calculator">KATARA</a></p>
        </div>
      </div>
    </div>
